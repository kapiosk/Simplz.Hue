namespace Simplz.Hue.Core.Services;
...
public sealed class HueService
{
    ...
    public async Task ApplyDimmerAsync(Guid roomId, double brightness)
    {
        _localHueApi ??= await GetHueApiAsync();
    ...
    public async Task TurnOnAsync(Guid roomId)
    {
        _localHueApi ??= await GetHueApiAsync();
    ...
    public async Task TurnOffAsync(Guid roomId)
    {
        _localHueApi ??= await GetHueApiAsync();
    ...
    public async Task<List<Models.LightStatus>> GetRoomLightsStatus(Guid roomId)
    {
        _localHueApi ??= await GetHueApiAsync();
        var room = await _localHueApi.GetRoomAsync(roomId);
        var lights = await _localHueApi.GetLightsAsync();
    ...
    public async Task<List<Models.Room>> GetGroupsAsync()
    {
        _localHueApi ??= await GetHueApiAsync();
    ...
    public async Task<List<Models.Scene>> GetScenesAsync()
    {
        _localHueApi ??= await GetHueApiAsync();
    ...
    public async Task ApplySceneAsync(Guid sceneId, Guid roomId)
    {
        _localHueApi ??= await GetHueApiAsync();
        var scene = await _localHueApi.GetSceneAsync(sceneId);
        var lightIds = await GetLightIdsAsync(roomId);
